...
CONTACT_US = EQUEST_LOGO_HTML + '<p style="color:black">FULL NAME: <span style="color:blue;">{0}</span><br><p style="color:black">INQUIRY TYPE: <span style="color:blue;">{1}</span></p><br><p style="color:black">PROFESSION: <span style="color:blue;">{2}</span></p><br>\
...
TRANSACTION_FAILURE_HTML = EQUEST_LOGO_HTML + '<p>Hi<br><p>Your transaction for course <b>{course}</b> has been declined due to a server error.Please mail this issue with your transaction ID <b>({txnid})</b> to our support team at <a href="mailto:equestsupport@qcin.org?Subject=Transaction Server Error">equestsupport@qcin.org</a> .</p>'
